                    ...
                    <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                        <div class="align-middle inline-block min-w-full shadow-md overflow-hidden sm:rounded-lg border-b border-gray-200">
                            <table class="min-w-full">
                                <thead>
                                    <tr>
                                        <th class="px-6 py-4 border-b border-purple-200 bg-purple-50 text-left text-xs leading-4 font-bold text-purple-600 uppercase tracking-wider">Nama / NIP</th>
                                        ...
                                        <tr class="hover:bg-purple-50 transition-colors duration-150">
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="flex items-center">
                                                    <div class="flex-shrink-0 h-10 w-10">
                                                        <img class="h-10 w-10 rounded-full shadow-sm object-cover" src="https://ui-avatars.com/api/?name=<?= urlencode($teacher['full_name']); ?>&background=random" alt="" />
                                                    </div>
                                                    <div class="ml-4">
                                                        <div class="text-sm leading-5 font-bold text-gray-800"><?= $teacher['full_name']; ?></div>
                                                        <div class="text-xs leading-5 text-gray-500 bg-gray-100 px-2 py-0.5 rounded inline-block mt-1">NIP: <?= $teacher['identification_number']; ?></div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="text-sm leading-5 font-medium text-gray-900"><?= $teacher['username']; ?></div>
                                                <div class="text-xs leading-5 text-gray-500"><?= $teacher['email']; ?></div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                            ...
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                                                <div class="flex space-x-3">
                                                    <a href="<?= BASEURL; ?>/admin/teachers_edit/<?= $teacher['id']; ?>" class="text-indigo-600 hover:text-indigo-900 transition-colors" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    
                                                    <a href="<?= BASEURL; ?>/admin/user_toggle_active/<?= $teacher['id']; ?>/teachers" class="<?= $teacher['is_active'] ? 'text-orange-500 hover:text-orange-700' : 'text-green-600 hover:text-green-800'; ?> transition-colors" title="<?= $teacher['is_active'] ? 'Nonaktifkan' : 'Aktifkan'; ?>">
                                                        <i class="fas <?= $teacher['is_active'] ? 'fa-ban' : 'fa-check-circle'; ?>"></i>
                                                    </a>

                                                    <button onclick="openResetPasswordModal(<?= $teacher['id']; ?>, '<?= $teacher['username']; ?>')" class="text-blue-600 hover:text-blue-900 transition-colors" title="Reset Password">
                                                        <i class="fas fa-key"></i>
                                                    </button>

                                                    <a href="<?= BASEURL; ?>/admin/teachers_delete/<?= $teacher['id']; ?>" class="text-red-600 hover:text-red-900 transition-colors" title="Hapus" onclick="return confirmAction(event, this.href, 'Hapus Guru?', 'Data guru akan dihapus permanen!')">
                    ...
                    <div class="mt-6 flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 shadow-sm rounded-lg">
                        <div class="flex flex-1 justify-between sm:hidden">
                            <?php if ($data['pagination']['current_page'] > 1): ?>
                                <a href="<?= BASEURL; ?>/admin/teachers?page=<?= $data['pagination']['current_page'] - 1; ?><?= $data['pagination']['keyword'] ? '&keyword=' . $data['pagination']['keyword'] : ''; ?>" class="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Previous</a>
                            <?php else: ?>
                            ...
                            <?php if ($data['pagination']['current_page'] < $data['pagination']['total_pages']): ?>
                                <a href="<?= BASEURL; ?>/admin/teachers?page=<?= $data['pagination']['current_page'] + 1; ?><?= $data['pagination']['keyword'] ? '&keyword=' . $data['pagination']['keyword'] : ''; ?>" class="relative ml-3 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">Next</a>
                            <?php else: ?>
                        ...
                        <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                            ...
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    <!-- Previous -->
                                    <?php if ($data['pagination']['current_page'] > 1): ?>
                                        <a href="<?= BASEURL; ?>/admin/teachers?page=<?= $data['pagination']['current_page'] - 1; ?><?= $data['pagination']['keyword'] ? '&keyword=' . $data['pagination']['keyword'] : ''; ?>" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                                    ...
                                    <?php for($i = 1; $i <= $data['pagination']['total_pages']; $i++): ?>
                                        <a href="<?= BASEURL; ?>/admin/teachers?page=<?= $i; ?><?= $data['pagination']['keyword'] ? '&keyword=' . $data['pagination']['keyword'] : ''; ?>" 
                                    ...
                                    <!-- Next -->
                                    <?php if ($data['pagination']['current_page'] < $data['pagination']['total_pages']): ?>
                                        <a href="<?= BASEURL; ?>/admin/teachers?page=<?= $data['pagination']['current_page'] + 1; ?><?= $data['pagination']['keyword'] ? '&keyword=' . $data['pagination']['keyword'] : ''; ?>" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Tambah Guru Baru</h3>
                        <div class="mt-4">
                            <form id="addForm" action="<?= BASEURL; ?>/admin/teachers_add" method="POST">
                                <div class="grid grid-cols-1 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="nip" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm">
                                    </div>
                                    <div>
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Reset Password Guru</h3>
                        <div class="mt-2">
...
<script>
    ...
    function openResetPasswordModal(userId, username) {
        ...
        // Set action URL
        form.action = "<?= BASEURL; ?>/admin/user_reset_password/" + userId + "/teachers";
